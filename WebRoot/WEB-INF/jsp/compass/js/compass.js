...
//上升、下降箭头
var raise = '<i class="raiseIcon"></i>';
var lower = '<i class="lowerIcon"></i>';
...
function queryDataBydate(dataType, dateValue, city) {
  ...
  if(dataType && dateValue && city){
      $.ajax({
        type: "get",
        url: GUrl + "/dataApiQuery/luopan_home_data?date_type=" + dataType + "&date_value=" + dateValue + "&city=" + city,
        dataType: "json",
...
function cityList() {
  $.ajax({
    url: GUrl + '/dataApiQuery/luopan_home_city',
    type: 'get',
    dataType: 'json',
    success: function (res) {
  ...
  dateList.addEventListener('click',function(ev){
      ...
      while(target !== dateList ){
          if(target.tagName.toLowerCase() == 'tr'){
            ...
            window.location.href="area.html?dataType="+dataType+"&dateValue="+riqi+"&topic_type="+topic_type+"&city="+city;
            break;
          }
...
function check4Data(className,value, huanbi,topic_type) {
  ...
  if (value) {
      ...
      if(cN!=null && cN!=""){
          if(cN==".pduan_atc_lv" || cN==".gg_shequ_zhanbi" || cN==".jf_shequ_rate" || cN==".bind_task_rate" || cN==".bind_time_rate"){
    ...
    $(className + " td:nth-child(3)").text("-");
  }
  if (Number(huanbi) > 0) {//以下 td前面的空格不要删除
    $(className + " td:nth-child(4)").text(huanbi + "%").removeClass("text-green").addClass("text-red").append(raise);
  } else if (Number(huanbi) < 0) {
    $(className + " td:nth-child(4)").text(huanbi + "%").removeClass("text-red").addClass("text-green").append(lower);
  }else if(Number(huanbi) == 0){
    if(value>0){
...
function check3Data(className,value, huanbi,topic_type) {
  ...
  if (Number(huanbi) > 0) {//以下 td前面的空格不要删除
    $(className + " td:nth-child(3)").text(huanbi + "%").removeClass("text-green").addClass("text-red").append(raise);
  } else if (Number(huanbi) < 0) {
    $(className + " td:nth-child(3)").text(huanbi + "%").removeClass("text-red").addClass("text-green").append(lower);
  }else if(Number(huanbi) == 0){
    if(value>0){
