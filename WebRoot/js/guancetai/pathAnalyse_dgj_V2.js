        ...
        hide: function (type) {
            ...
            function effectSwitching() {//效果切换
                ...
                for (var i = 0; i < allPage.length; i++) {
                    ...
                    var a = "left:64px";
                    var b = "left:40px";
                    li.innerHTML = ap['name'] + `<div class="" style="` + (ap['name'].length > 4 ? a : b) + `"><div class="top"></div><div class="bot"></div></div><h5>` + number + `</h5>`
                    ul.appendChild(li)
...
function calendarshow() {
    $('.c-datepicker-picker').css({'left': '300px'});
...
//  下面三行是为了默认进入后样式显示第一个
    /*  li[0].className = "";
     li[0].firstElementChild.className = "";
     li[0].firstElementChild.style.display = "none";*/
    for (var i = 0; i < li.length; i++) {
        function pageJumpFun() {
//   树形菜单跳转页面使用
            for (var j = 0; j < li.length; j++) {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                for (var i = 0; i < allPage1.length; i++) {
                    ...
                    li.innerHTML = ap['name'] + `<div class="" style="` + flag + `"><div class="top"></div><div class="bot"></div></div><h5>` + number + `</h5>`;
                    ul.appendChild(li)
...
function downTreeMenu(ids, that) {
    ...
    for (var i = 0; i < loopValue2.length; i++) {
        ...
        li.innerHTML = loop["name"] + `<div class=""><div class="top"></div><div class="bot"></div></div></div><h5>` + number + `</h5>`
        ul.appendChild(li);
